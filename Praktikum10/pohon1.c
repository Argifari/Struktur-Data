...
int NbElm(bintree P) {
    ...
    if (IsEmptyTree(P)) {
        ...
        return 1 + NbElm(GetLeft(P));
    }else if (IsUnerRight(P)) {
        return 1 + NbElm(GetRight(P));
    }else if (IsBiner(P)) {
...
int NbDaun(bintree P) {
    ...
    if (IsEmptyTree(P)) {
        ...
        return NbDaun(GetLeft(P));
    }else if (IsUnerRight(P)) {
        return NbDaun(GetRight(P));
    }else if (IsBiner(P)) {
...
int Tinggi(bintree P) {
    ...
    if (IsEmptyTree(P)) {
        ...
        return max2(Tinggi(GetRight(P)),Tinggi(GetLeft(P)));
    }
    else {
        return max2(1 + Tinggi(GetRight(P)), 1 + Tinggi(GetLeft(P)));
    }
}
...
boolean IsSkewLeft (bintree P) {
    ...
    if (IsUnerLeft(P)) {
        return true && IsSkewLeft(GetLeft(P));
    }else {
        return false;
    }
}
...
boolean IsSkewRight (bintree P) {
    ...
    if (IsUnerRight(P)) {
        return true && IsSkewRight(GetRight(P));
    }else {
        return false;
    }
...
int LevelX(bintree P, infotype X) {
    ...
    if (IsEmptyTree(P)) {
        ...
        if (akar(P) == X) {
            ...
            levelRight = LevelX(GetRight(P),X);
            levelLeft = LevelX(GetLeft(P),X);
            if (levelRight != 0) {
                return levelRight + 1;
            }else if (levelLeft != 0) {
                return levelLeft + 1;
...
char Modus(bintree P) {
    ...
    if (!IsEmptyTree(P)) {
        ...
        if (!IsEmptyTree(GetLeft(P))) {
            leftHasil = Modus(GetLeft(P));          
            countLeft = CountX(P, leftHasil);        
            if (countLeft > maks) {
                maks = countLeft;
                hasil = leftHasil;
            }
        ...
        if (!IsEmptyTree(GetRight(P))) {
            rightHasil = Modus(GetRight(P));         
            countRight = CountX(P, rightHasil);      
            if (countRight > maks) {
                maks = countRight;
                hasil = rightHasil;
            }
